...
if (await email.count()) {
  ...
  await p.locator('text=Sign in').last().click().catch(()=>{});
  await p.waitForTimeout(4000);
}
if (route !== '/') { await p.goto(`http://localhost:8081${route}`, { waitUntil: 'networkidle' }); await p.waitForTimeout(2500); }
await p.screenshot({ path: out, fullPage: true, type: 'jpeg', quality: 88 });
console.log('landed:', new URL(p.url()).pathname, errs.length ? `ERRORS: ${errs.slice(0,2).join(' | ')}` : 'clean');
await b.close();
