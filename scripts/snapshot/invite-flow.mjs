...
const link = await p.locator('a[href*="/welcome"], code, input[readonly]').first().innerText().catch(() => null);
console.log('invite result:', (link || '').slice(0, 160));
const body = await p.locator('body').innerText();
const m = body.match(/https?:\/\/[^\s]*(?:welcome|auth\/callback)[^\s]*/);
if (!m) { console.log('NO LINK FOUND'); console.log(body.slice(0, 900)); await b.close(); process.exit(0); }
...
await p2.selectOption('select[aria-label="You set out from"]', { index: 1 }).catch((e)=>console.log('select failed', e.message));
await p2.waitForTimeout(2200);
await p2.check('input[value="street"]').catch(()=>{});
await p2.waitForTimeout(400);
await p2.addStyleTag({ content: 'nextjs-portal,[data-nextjs-dev-tools-button]{display:none!important}' }).catch(()=>{});
await p2.screenshot({ path: out, fullPage: true, type: 'jpeg', quality: 88 });
console.log('landed:', new URL(p2.url()).pathname, errs.length ? `ERRORS: ${errs.join(' | ')}` : 'clean');
