...
const EMAIL = { client: 'client@example.test', admin: 'admin@example.test', auditor: 'auditor@example.test' }[role];
...
await p.addStyleTag({ content: 'nextjs-portal,[data-nextjs-dev-tools-button]{display:none!important}' }).catch(()=>{});
await p.screenshot({ path: out, fullPage: true, type: 'jpeg', quality: 88 });
console.log('landed:', new URL(p.url()).pathname, errs.length ? `ERRORS: ${errs.join(' | ')}` : 'clean');
await b.close();
