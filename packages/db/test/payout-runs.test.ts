...
describe('running a payout', () => {
  ...
  it('cannot pay one audit twice, across every run there has ever been', async () => {
    await withDatabase(async (db) => {
      ...
      // Arranged as postgres, not as the admin. What is being tested here is
      // the unique partial index — a schema invariant that holds against
      // anything, including a bad migration script running as service_role.
      // A signed-in admin cannot reach this table at all any more (payout runs
      // are built by build_payout_run), and refusals.test.ts covers that; going
      // through `as(admin)` would now fail at the grant and stop testing the
      // index at all.
      let message = '';
      try {
        await db.arrange(
