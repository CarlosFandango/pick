...
describe('gate configuration', () => {
  ...
  it('has exactly one row per trigger, so a trigger cannot mean two things', async () => {
    await withDatabase(async (db) => {
      // As postgres: the gates are seeded, one row per trigger, and an admin
      // edits them rather than adding them — so no signed-in role holds INSERT
      // and going through `as(admin)` would fail at the grant and stop testing
      // the index. What is asserted here is the index.
      let message = '';
      try {
        await db.arrange("insert into review_gate (trigger) values ('manual')");
      } catch (error) {
