...
export function PayableList({ payable }: { payable: readonly PayableRow[] }) {
  if (payable.length === 0) {
    return <p style={{ fontSize: fontSize.sm, color: color.muted }}>Nothing is owed right now.</p>;
  }
    ...
    <section style={{ ...card, padding: 18 }}>
      ...
      <ul style={{ listStyle: 'none', margin: '12px 0 0', padding: 0 }}>
        {[...ready, ...held].map((line) => (
          ...
          >
            <span style={{ fontFamily: mono, fontSize: fontSize.xs }}>{line.reference}</span>
            <span style={{ fontSize: fontSize.sm }}>{line.auditorName}</span>
            {line.gate === 'hold' ? (
...
export function RunList({
  ...
  if (runs.length === 0) {
    return <p style={{ fontSize: fontSize.sm, color: color.muted }}>No runs yet.</p>;
  }
        ...
        >
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 10 }}>
              <span
                style={{ fontFamily: mono, fontWeight: fontWeight.bold, fontSize: fontSize.sm }}
              >
                {run.reference}
              </span>
              <span
