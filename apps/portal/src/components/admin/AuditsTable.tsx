...
export function AuditsTable({ audits }: { audits: readonly AdminAuditRow[] }) {
  if (audits.length === 0) {
    return <p style={{ fontSize: fontSize.sm, color: color.muted }}>Nothing booked yet.</p>;
  }
      ...
      <tbody>
        {audits.map((audit) => (
          <tr key={audit.id}>
            ...
            <td style={{ ...cell, fontWeight: fontWeight.semibold }}>{audit.charityName}</td>
            <td style={cell}>{audit.auditTypeLabel}</td>
            <td style={{ ...cell, fontFamily: mono, fontSize: fontSize.xs }}>{audit.postcode}</td>
